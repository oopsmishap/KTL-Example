...
if(NOT DEFINED WDK_WINVER)
    set(WDK_WINVER "0x0601" CACHE STRING "Default WINVER for WDK targets")
elseif(DEFINED TARGET_WINVER)
    set(WDK_WINVER ${TARGET_WINVER} CACHE STRING "Default WINVER for WDK targets" FORCE)
endif()

if(NOT DEFINED WDK_NTDDI_VERSION)
    set(WDK_NTDDI_VERSION "0x06010000" CACHE STRING "Specified NTDDI_VERSION for WDK targets if needed")
elseif(DEFINED TARGET_NTDDI_VERSION)
    set(WDK_NTDDI_VERSION ${TARGET_NTDDI_VERSION} CACHE STRING "Specified NTDDI_VERSION for WDK targets if needed" FORCE)
endif()
...
if(DEFINED ENABLE_WDK_EXTENDED_CPP_FEATURES)
    set(WDK_EXTENDED_CPP_FEATURES ${ENABLE_WDK_EXTENDED_CPP_FEATURES} CACHE BOOL "Enable extended C++ features (exceptions, etc) for WDK targets" FORCE)
else()
    option(WDK_EXTENDED_CPP_FEATURES "Enable extended C++ features (exceptions, etc) for WDK targets" OFF)
endif()
...
message(STATUS "WDK_EXTENDED_CPP_FEATURES: ${WDK_EXTENDED_CPP_FEATURES}")
...
function(wdk_add_driver _target)
    # Parse arguments - rename to WDK_ARG to avoid conflict with global WDK_EXTENDED_CPP_FEATURES
    cmake_parse_arguments(WDK_ARG "EXTENDED_CPP_FEATURES;NO_EXTENDED_CPP_FEATURES" "CUSTOM_ENTRY_POINT;KMDF;WINVER" "" ${ARGN})

    # Determine if we should enable extended C++ features
    if(WDK_ARG_NO_EXTENDED_CPP_FEATURES)
        # Explicitly disabled for this target
        EXTENDED_CPP_FEATURES_OFF(${_target})
    elseif(WDK_ARG_EXTENDED_CPP_FEATURES)
        # Explicitly enabled for this target
    ...
    add_custom_command(
        ...
        COMMAND "${WDK_SIGNTOOL}" sign /fd SHA1 /f "${WDK_PFX}" "$<TARGET_FILE:${_target}>"
        VERBATIM
    )
endfunction()
...
function(wdk_add_library _target)
    # Parse arguments
    cmake_parse_arguments(WDK_ARG "EXTENDED_CPP_FEATURES;NO_EXTENDED_CPP_FEATURES" "KMDF;WINVER" "" ${ARGN})

    # Determine if extended C++ features should be enabled
    if(WDK_ARG_NO_EXTENDED_CPP_FEATURES)
        # Target explicitly disabled extended C++ features
        EXTENDED_CPP_FEATURES_OFF(${_target})
    elseif(WDK_ARG_EXTENDED_CPP_FEATURES)
        # Target explicitly enabled extended C++ features
        EXTENDED_CPP_FEATURES_ON(${_target})
    else()
        # Use global setting
        if(WDK_EXTENDED_CPP_FEATURES)
    ...
    if(DEFINED WDK_ARG_KMDF)
        target_include_directories(${_target} SYSTEM PRIVATE "${WDK_ROOT}/Include/wdf/kmdf/${WDK_ARG_KMDF}")
    endif()
endfunction()